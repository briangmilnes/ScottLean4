...
status: pending
supersedes: plans/r0030-plan-from-orchestrator-to-agent2-powerdomain-universal.md
---

# r0031 agent4 — Lemma 30 and Lemma 28's remaining conjuncts

**Why this stream matters most for what follows.** Lemma 28's `×` conjunct is the
one hypothesis Lemma 24 still lacks, and Lemma 24 feeds Theorem 25, which feeds
Theorems 26 and 27. Four of the thirteen remaining results sit behind it, so
prove the `×` conjunct **first**, before the powerdomain ones, and report it as
soon as it lands.
...
0. **Lemma 28's `×` conjunct, first.** `IsRepresentable (Set ℕ) F` for `F` the
   product operator, in the same shape as `lem23`. This unblocks Lemma 24 →
   Theorem 25 → Theorems 26 and 27, so land and report it before the rest.
...
Write `reports/r0030-report-from-agent4-to-orchestrator-powerdomain-universal.md`
