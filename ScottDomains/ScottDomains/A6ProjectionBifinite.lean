...
`LemThirty.Theorem29SecondAtDomains` alone, with no bounded-completeness binder and
...
`LemThirty.Theorem29Normal`. The development's own bounded-completeness obstruction,
the second of the two r0046 recorded, is gone. -/
theorem lemma_30_atV_of_thm29Normal (h : Theorem29Normal) : LemThirty.Lemma30AtV :=
