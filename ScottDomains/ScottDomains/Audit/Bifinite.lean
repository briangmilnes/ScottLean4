...
`JungNets.lean` ends with two declarations,
    ...
    lemma217_of_thm137       (h : Thm137 D) (hAlg) (hCount) {a₁ a₂ : D}
                             (ha₁ : IsCompactElement a₁) (ha₂ : IsCompactElement a₂) :
                             (minimalUpperBounds (compacts D) {a₁, a₂}).Finite

    propertyM_pairs_of_thm137 (h : Thm137 D) (hAlg) (hCount) :
...
of `JungNets.propertyM_pairs_of_thm137` itself, which is the sharpest form the
...
`propertyM_pairs_of_thm137` is mentioned nowhere in the package except at its own
declaration, and `lemma217_of_thm137`'s only mention is inside
`propertyM_pairs_of_thm137`'s proof. So the pair is uncited as a pair, and
...
/-- **`propertyM_pairs_of_thm137` from `lemma217_of_thm137`.** One application;
this is `JungNets.propertyM_pairs_of_thm137`'s own proof term. -/
theorem propertyM_pairs_from_lemma217 (h : Thm137 D) (hAlg : IsAlgebraic (ScottHom D D))
