...
| 2e | — **unspecified**: paper properties with **no Lean statement at all** | **62 of 239 — measured, r0040**, five agents over all five sections; see [`../analyses/property-coverage.2026-0808-11:59.orchestrator.md`](../analyses/property-coverage.2026-0808-11:59.orchestrator.md). **Only 2 are numbered conjuncts** — Theorem 7's second sentence and its `D ⊸ E` twin, both about effective presentations, and `EffectivePresentation` turns out never to be instantiated at *any* type. The other 60 are unnumbered prose: §4's morphism-level algebra (11), worked-example objects never constructed such as `N⊥`, `ω`, `Q` (8), no functorial action on a powerdomain (2), §7.2's λ-calculus equations (5), and proof-sketch steps the development deliberately skips | 91 of 93 numbered conjuncts **are** stated, so this is not incompleteness in the paper's mathematics; it is the paper's examples, remarks and sketches. The previous entry here read "≥ 1" because row 3's list was assembled from claims the development *proves* and so could not record a gap | **239** |
