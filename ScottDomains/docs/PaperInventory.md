...
| 4.5 | Thm | 14 | Equivalent characterizations of an (algebraic/BC) domain | ✗ prove — **statement recovered** (r0032), still `sorry`, and **the obstacle is mathematical, not definitional**. An earlier draft of this row, and the r0034 plan, claimed supplying `IsBifiniteViaProjections` was enough: it removes the `P ↔ P` degeneracy, but what remains is Plotkin's SFP characterization. Four gaps, recorded in `thm14`'s docstring (r0034): (1) the whole `FpLattice` section of `FinitaryProjectionPoset.lean` sits under `variable [Domain α]`, which is exactly what the forward direction must *conclude*; (2) `Fp(D)`'s compactness results use `(fpBasis q).Finite` while `finiteImageProjections` needs `(Set.range ⇑q).Finite`, and the bridge `Set.range ⇑(toFp hN) = N` does not exist; (3) `IsLUB` in `↥(Fp α)` is strictly weaker than `IsLUB … ScottHom.id` in `ScottHom α α`; (4) two finite-combinatorial lemmas are missing. Schedule gap 2 first — gaps 1 and 3 both route through it |
