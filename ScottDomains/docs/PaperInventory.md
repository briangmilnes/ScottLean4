...
| 3 | **Unnumbered prose claims** | **146 stated by the paper**, of which the development proves 91 (r0040, measured section by section: §2/§3 45, §4 37, §5 17, §6 19, §7 28). **The figure previously here was 13, and it was not a count of the paper** — the list was assembled from claims the development proves, every entry naming a Lean declaration, so a claim with no declaration could never enter it. It was then used as a coverage denominator. r0038's correction of that list (strike one claim the paper does not make, add two §4.1 claims it does) stands and is folded in | 60 unstated — see row 2e | **146** |
