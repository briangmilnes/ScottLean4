...
| 2f | — **stated in prose only**: claims the development asserts but never puts under the kernel | **3** — (1) Theorem 26 is false for a signature admitting arity 0; (2) `Colimit.Thm29Second` is false without `[Domain E]`, where `countable_compacts_of_reflects` is checked but the step from it is not; (3) the `⊗`/`⊕` counterexample refuting Lemma 28's closure reading, 30 lines at `CombinatorRep.lean:504–533` whose own text says "a hand computation, not Lean-checked" | all 3 are cheap to put under the kernel; the `lem10_smash` and Theorem 16 precedents say to do it | — |
