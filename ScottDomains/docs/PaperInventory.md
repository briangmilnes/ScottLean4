...
## What r0049 changed (2026-0810)

Six streams, all merged, build 1372 jobs at 0 errors / 0 warnings / `sorry` 0.
Rows 2c, 2f, 2h and 5 below are corrected in place; this is the summary.

| # | Result | Evidence |
| -- | ----- | -------- |
| 1 | **Theorem 26 is false as printed** | `R49.Agent7.not_thm26Printed_of_two_zero_arities`. Printed defects **9 → 10**; `hs` is a repair, not a defect of ours |
| 2 | **`FpImagesBifinite V` discharged** | `R49.Agent6.fpImagesBifinite_V`, no hypotheses, no binders. `Lemma30AtV` goes **arity 3 → 1** |
| 3 | **`Thm29Normal` closed at finite bases** | `R49.Agent5.thm29Normal_finiteBasis`; `A∞` is universal for the finite pointed posets. Open only for infinite `K(E)` |
| 4 | **`StepFunctionsDecidable` restated** | over `IsStepEnumeration`, quantified existentially, so it names no guard its hypotheses do not determine. Old → new is a **weakening** |
| 5 | **The recursion theory supplied** | the `Finset (ℕ × ℕ)` coding is `Primrec`; §3.2's two conditions **decide boundedness and compute the join's index**. One item left: `RecursiveNormal` for `K(D → E)` |
| 6 | **A standing staleness check** | `scripts/a8-claim-check.sh`, 74-obligation baseline, 12.3 s |

**Two recorded blockers were re-derived and found overstated** — r0047's
projection-image obstruction (the argument needs a `G`-fixed point below each
`x`, which a finite image forces; it does not need the missing idempotence) and
r0047's "route through Theorem 25 is closed" (the refutations quantify
*universally* over subposets while `Thm29Normal` quantifies *existentially* over
embeddings, so they close **extension**, not **construction**). That is **seven
overstated blockers in four rounds**, and it is now the most reliable predictor
of where work is available.

**Two corrections ran the other way, toward the paper**: r0044's row 3 binder is
no defect, and the step-function guard sharpens to *the paper tests neither*,
because on the paper's own `(N, s)` index the join always exists.
...
| 2c | — **qualifications on three of the 24** | **Lem 9**: four conjuncts hold as printed; items 3 and 5 are **false as printed** and are kernel-checked negations (`lem9_3_printed_false`, `lem9_5_printed_false`), with the corrected laws proved. **Lem 17**: 10 of 10, but the `→` and `◦→` conjuncts carry `[BoundedComplete β]` from the step-function decomposition — stronger than the paper states, and §6 exists precisely to avoid bounded completeness. **Thm 26**: proved with `hs : ∀ i, 0 < s i`. **r0049 settled this row and moved it out of the qualification class**: the printed theorem is **false**, kernel-checked as `R49.Agent7.not_thm26Printed_of_two_zero_arities`, for any signature with **two or more** 0-ary slots — including the paper's own worked `(2,0,0,0,0,0)`. So `hs` is a **repair of a printed defect**, alongside Lem 9 items 3 and 5, not a defect of ours. Two corrections to what stood here: the refutation needs *two* nullary slots, not one, and it does **not** use `fst(ψ(x)) = x` — the argument previously recorded in `Combinator.lean:60–72` is invalid, since `isAlgEmbedding_const_of_subsingleton` shows two one-point algebras do land on the same subalgebra. **Lem 10** carries no qualification | — | — |
| 2d | — **status of the four remaining**, after r0042 | **Superseded on the first clause: the development has had `sorry` 0 since r0042 and Theorem 18 is proved** (row 6). Retained for the Corollary 1.36 history below. As written: **Thm 18** was then the development's only remaining `sorry`, and rested on **one** named proposition. Jung's **Corollary 1.36 is proved** (r0042, `JungCor136`) — not by his route, which goes through Proposition 1.22 and a retraction pair, but by indexing the approximating family below `cap e` rather than below `id`, so the deflation condition is built into the index and the object r0037 measured as unavailable is never wanted. What remains is `JungNets.Thm137`, and r0042 reduced it further: `Iwamura.thm137Chains_iff_thm137` and `Thm18.thm18_of_thm137Chains_and_cor136` show only **chain** infima are ever spent, and `JungBicomplete.hasChainInfima_of_forall_isOrdinalCodirected` shows Jung's Corollary 1.3 is *free* for chains. **`Thm137Chains` now factors as the retraction onto `D′ = A ∪ αᵒᵖ` plus the `g_β` family, and nothing else.** The assembly is a library theorem (`ScottDomains.Thm18`), not only a script, and `thm18`'s statement was audited against printed page 33 and found correct in all four constituents. **Lem 28** is **7 of 9 over the paper's own `Dyadic.U` with no hypothesis** (`→, ⇸, ×, ⊗, +, ⊕, ()⊥`), up from 0 before r0037; `()♯` and `()♭` remain. **Thm 29**'s second sentence is reduced to the single proposition `LemThirty.Thm29Normal`, kernel-checked in ~60 lines. **Lem 30** is stated at ten conjuncts over `V` and remains 0 of 10, but the §7.3 schemes are measured to be generic in the carrier, so it is ten instantiations plus ten retraction pairs, not ten fresh proofs | — | — |
| 2e | — **unspecified**: paper properties with **no Lean statement at all** | was **62 of 239** when r0040 measured it (five agents over all five sections; see [`../analyses/property-coverage.2026-0808-11:59.orchestrator.md`](../analyses/property-coverage.2026-0808-11:59.orchestrator.md)). **r0041 closed about 40 of them** by building the four constructions they clustered on: the flat cpo (`Flat`, an `inductive` — `WithBot` keeps the carrier's order and `WithBot Bool` is *linear*, where the flat order needs the two non-⊥ points incomparable), the morphism-level algebra at 11 of 11 (`Morphism`), the powerdomain functor `f♮`/`f♯`/`f♭` with both functor laws (`PowerdomainMap`), and instantiated effective presentations (`Effective/`) | **26**, and **zero of them numbered**. **This is measured, not derived** — r0043 re-checked all 62 rows against the tree (five agents, own-area `N` rows only; see [`../analyses/property-coverage-remeasure.2026-0808-16:55.orchestrator.md`](../analyses/property-coverage-remeasure.2026-0808-16:55.orchestrator.md)). 36 closed, 26 remain; the "≈22" that stood here was 62 − 40 by subtraction and was low by 4. The 36 break down 33 `S+P`, 2 `S≠`, 1 `S+H`. The residue is concentrated rather than spread: **13 of the 26 are §7 alone**, where re-measurement moved *nothing* — r0041 made `X ≅ N⊥ + (X → X)` writable for the first time by supplying `Flat.NatBot`, and no statement writes it, so that round changed **expressibility, not statedness**. The other 13 are three missing carriers (ℝ, ℚ) with two meta-claims blocked behind them, `N⊥`'s two function spaces, three order facts, a Figure 3c poset nobody has built, and one "subset" claim without `⊆` | **239** |
| 2f | — **stated in prose only**: claims the development asserts but never puts under the kernel | **2 after r0049**, down from 3 — (1) `Colimit.Thm29Second` is false without `[Domain E]`, where `countable_compacts_of_reflects` is checked but the step from it is not; (2) the `⊗`/`⊕` counterexample refuting Lemma 28's closure reading, 30 lines at `CombinatorRep.lean:504–533` whose own text says "a hand computation, not Lean-checked". **Closed by r0049**: Theorem 26's arity-0 falsity is now under the kernel. The instructive part is that the prose was **right in its conclusion and wrong in its argument** for three rounds, and only putting it under the kernel separated the two | both remaining are cheap to put under the kernel; the `lem10_smash`, Theorem 16 and now Theorem 26 precedents say to do it | — |
| 2g | — **why these two rows exist** | `sorry` counts holes in *stated* theorems. A result nobody wrote down produces no warning, no count and no build signal, so rows 6 and 2e measure different things and neither substitutes for the other. A development can reach **0 `sorry` while leaving paper results unstated**, and this one is two results away from exactly that | — | — |
| 2h | — **mis-specified**: stated, but not as the paper states it | **13 defects of 18 `S≠` rows** (r0044 agents 1–2; see [`../analyses/specification-defects.2026-0808-21:05.orchestrator.md`](../analyses/specification-defects.2026-0808-21:05.orchestrator.md)). Split **12 under-specified, 1 incorrectly specified, 5 deliberately divergent** — the last being repairs of the paper's own printed defects, which are correct work and not counted. The `S≠` label had merged all three, so the 18 previously quoted overstated our defects by 39%. **The structural finding: 9 of the 12 are the added-binder shape**, stating the paper's conclusion under *stronger hypotheses than the paper assumes*. Only one is a missing conjunct. r0040 and r0043 both hunted missing conjuncts, which is why this went unseen — an added binder reads as faithful transcription until you compare them. **r0049 re-derived the row population against the built environment rather than inheriting this table, and it moved: 13 defects of ours → 8, and the `S≠` census 18 → 14.** Two rows closed in r0049 (`Kleene.sSup_recoverAt` at the paper's hypotheses, and the p9b stabilizing index), two had closed in r0047 and were never re-counted, and Theorem 26 was reclassified *ours → repair*. Two inherited entries were also wrong in the paper's favour: row 3's `[BoundedComplete β]` is **not** a defect, since printed Theorem 7 assumes bounded completeness; and rows 8–12 are **one** obstruction, not five — whether the closure image is algebraic | 8 | 14 |
| 2i | — **vacuously specified**: labelled `S+P`, hypotheses unused | **7** — 6 from r0044 agents 3–5, plus one from r0045 by a **second and independent mechanism**: `Effective.PreservesRecursivePresentation` quantifies over a `γ` unrelated to its `α` and `β`, so the identity closes it in one line, and its closure over `γ` is false. **That vacuity lives in the quantifier structure, not a field type**, so a reviewer checking only for `Classical.dec` passes it; both mechanisms are now a two-question check in [`StructuresVsTypeClassesVsPropsInLean4.md`](StructuresVsTypeClassesVsPropsInLean4.md). **r0047 swept for the second mechanism and found zero further instances** — `scripts/a3-r47-qvac-body.lean` builds the parameter graph and flags a conclusion parameter whose connected component contains no hypothesis parameter, returning **1 hit in 2,111 declarations, the known one**, at 100% precision with **recall cost 0** against a looser criterion scoring 6.7% (all 15 of its hits adjudicated by reading). Five controls print on every run, one of them a cross-instrument control showing r0045's `a5-freehyp` reports 50 free `Prop` rows and says **nothing** about this mechanism — confirming *by measurement* that the field-based sweeps would have passed it. The known instance was itself restated in r0047 to quantify over an **operator**, which is what the paper's sentence quantifies over; that restatement is a **strengthening**, running opposite to r0046's, and `preservesRecursivePresentation_arrowOp_iff` proves the new claim at the arrow equivalent to `Theorem7ArrowRecursive` — a relationship the old docstring asserted but the free-carrier statement could not even express. The first mechanism: `Classical.dec` inhabits `EffectivePresentation`'s decidability fields at no cost, so *every* domain has one and any theorem quantifying over it is vacuous — see [`StructuresVsTypeClassesVsPropsInLean4.md`](StructuresVsTypeClassesVsPropsInLean4.md). **`#lint unusedArguments` alone finds zero of them**: it exempts `_`-prefixed binders, and both originally-known cases are named `_d`/`_e`. Three complementary instruments were needed — unused-in-proof-term, **used-but-free** (which found all 4 new ones), and deletion-plus-reproof. A separate **26 declarations are *over*-hypothesized**, proving more than they claim; that is the opposite sign and is deliberately excluded | 6 | 1,773 |
| 2j | — **undischarged**: claims stated as `Prop`-valued `def`s that no theorem proves | **7 open after r0047** (8 strict) — **unchanged in number by that round, and that is the honest headline** (see [`../analyses/close-the-seven.2026-0809-12:35.orchestrator.md`](../analyses/close-the-seven.2026-0809-12:35.orchestrator.md)). Three of the seven now mean something different. **`Thm29Normal`'s route is a dead end**: Gunter's Lemma 24 at `M(A)` is *proved* (`R47.Agent1.lemma24_MPair`, the first proof anywhere — the printed Lemma 24 is not about `M(A)`, and the `M(A)` form is a remark with no theorem and no proof), but `HasNormalRealizations Ainf` is **refuted**, so r0046's sound implication has an unsatisfiable hypothesis. **`StepFunctionsDecidable`'s subject has a kernel-checked defect**: `IsCompactElement (ofPairs Q)` is *not* the boundedness test, since `sSup` on `ScottHom` is total and reads a junk value on an inconsistent `Q` — our transcription error, not the paper's. The domain theory it needed *is* proved (`bddAbove_stepsOf_iff`), and restating over the consistency-guarded enumeration — machinery already built — closes **four** claims, not three. Separately r0047 removed `[BoundedComplete β]` from six declarations including **`Domain (ScottHom α β)`**, which Goal A cannot see at all. Also **the `S+H` unfinished-proof count was 16 and is 12** — four rows had closed in r0042–r0045 and the tally was never decremented. The prior figure was **7 after r0046**, itself down from 10, re-derived on the merged tree (see [`../analyses/zero-props-zero-false-prose.2026-0809-11:40.orchestrator.md`](../analyses/zero-props-zero-false-prose.2026-0809-11:40.orchestrator.md)). Three are now **refuted and therefore resolved** — `Colimit.Thm29Second`, `PRep.Lemma28`, `LemThirty.Lemma30`, the last having been argued false in prose across two rounds and never proved so until `not_forall_lemma30`. **The metric itself was defective and is fixed**: a refutation produces `¬ Foo`, not `Foo`, so a claim proved false still scored as undischarged; `a6-summarize.py` now reports `refuted` as a third status and flags resolutions holding only at a **parameter instance** — which is why the strict count is 8, `Effective.PreservesRecursivePresentation` being resolved only at `γ := α` and `γ := Flat ℕ`. **`Thm29Normal` was never blocked on the unobtainable [Gun87]**: the proof is in `papers/Gunter 1987 Universal Profinite Domains.pdf` §5, already cited at `BifiniteUniversal.lean:47` for the construction, and r0046 reduced the claim to one named input, Gunter's Lemma 24 at `M(A)`. `StepFunctionsDecidable` is blocked on **one domain-theoretic lemma** — deciding `IsCompactElement (ofPairs Q)` — not on the two recursion-theoretic obstructions previously recorded, both of which were measured real and non-blocking. The prior figure was **10 after r0045**, itself down from 19, re-derived by re-running the detector over the built environment rather than by subtraction (see [`../analyses/discharge-nineteen.2026-0808-21:45.orchestrator.md`](../analyses/discharge-nineteen.2026-0808-21:45.orchestrator.md)). **7 claims are resolved**: 5 fully discharged (the four `PowerdomainMap.Rep.*` obligations, which were exactly `lemma28AtU_of''`'s four hypotheses, and `PRep.Lemma28AtU` with them) and **2 refuted** — `Colimit.Thm29Second` at `Flat (Set ℕ)` and `PRep.Lemma28` at `Flat Empty`, both kernel-checked. A further 3 are **discharged at `[Domain D]`** (Jung's Theorem 1.37 and its two weakenings), which is a real advance but leaves the general case open, and 1 is a false positive the detector cannot see (below). **The denominator was also inflated**: `Lemma28`/`Lemma28AtU` and `Lemma30`/`Lemma30AtV` are two schema/instance pairs, each one claim counted twice, so the real population was 17. Neither refutation convicts the paper — `Thm29Second` is false only because our transcription **dropped the word "domain"** from "any bifinite domain", which is r0044's added-binder defect running in reverse. The original count of **19** (r0044 agent6) included `StepFunctionsDecidable`. **This is a documented convention, not an accident** — `LemThirty.Thm29Normal`'s docstring says "Recorded as a `Prop` rather than a `sorry` … the statement is fixed and citable, and nothing asserts it." So **row 6's `sorry` 0 has a systematic 19-claim blind spot by design**, and the design was never recorded where a count would find it. Six carry a nonzero "proved by" count and every such theorem is a *reduction*, not a proof. Of 88 `Prop`-valued defs, 55 lack an unconditional proof, splitting 19 claims / 36 concepts; 55 would have been the inflated figure. Census alongside: **0 `axiom` declarations**, 0 `sorryAx`, **0 of 22 structures never instantiated** | 19 | 88 |
| 3 | **Unnumbered prose claims** | **146 stated by the paper** (r0040, measured section by section: §2/§3 45, §4 37, §5 17, §6 19, §7 28 — these sum to 146), of which the development **proves 70** and states in some form 86. **The figure here was 91 until r0044, and 91 was the *numbered-results* count** ("91 of 93 numbered conjuncts are stated") transcribed into the prose row; it exceeds the maximum possible by 5, since only 86 prose claims are stated at all. **The row's arithmetic is also structurally unsound and always was**: `146 − proved = unstated` assumes the categories are complementary, but `S+H`, `S≠` and `P` account for 16 prose rows sitting between "proves" and "unstated", so no assignment of three numbers can close it. Separately, **the figure before that was 13, and it was not a count of the paper** — the list was assembled from claims the development proves, every entry naming a Lean declaration, so a claim with no declaration could never enter it, and it was then used as a coverage denominator. r0038's correction of that list (strike one claim the paper does not make, add two §4.1 claims it does) stands and is folded in | **60 unstated.** This 60 is correct and only looks inconsistent because of the cross-reference: it is the prose-only `N` sum (62 less the two *numbered* `N` rows), whereas row 2e's 62 ranges over all 239 properties | **146** |
| 4 | Mathlib foundations reused | 12 | — | 12 |
| 5 | Modules / lines / theorems | **124 / 46097 / 2119** — measured on `main` after r0049, all six streams merged; build 1372 jobs, 0 errors, 0 warnings, `sorry` 0. r0049 added six modules: `Effective/A3StepDecidable.lean`, `Effective/A4Recursion.lean`, `A5Thm29Finite.lean`, `A6ProjectionBifinite.lean`, `A7Thm26Arity.lean`, `A7SneqRows.lean`. Previously **117 / 43481 / 2019** — measured on `main` after r0047, `Audit/` included; build 1364 jobs, 0 errors, 0 warnings. The environment reports **2,114 theorems** and **4,089 constants**, which measure different sets and must not be reconciled with this row. Previously **109 / 40830 / 1911** after r0046; build 1356 jobs. r0045 added five modules (the discharge round) and r0046 five more, among them `A2Thm29Universal.lean`, which reduces Theorem 29's normality claim to a single named input, and `Effective/A3StrictRecursive.lean`, which supplies the enumeration of `K(D ⊸ E)`. Three other counts measure different sets and **must never be reconciled with this one**: the environment reports **2,009 theorems** (adding `Prop`-valued class instances) and **3,908 constants** (adding defs, structures, projections and equation lemmas). Previously **99 / 36275 / 1735** with r0041 and r0042 merged. r0041 added 16 modules and r0042 six, among them `Iwamura.lean`, which carries **Iwamura's lemma and Markowsky's theorem — neither has ever been in Mathlib**. Previously 72 / 27892 / 1298 with r0037 merged, re-measured after r0038 by `scripts/lean-decls.py`, which `counts.sh` now calls: the grep rule it replaced counted declarations inside `/- … -/` block comments and docstring prose lines beginning "theorem"/"lemma", and missed `protected theorem`, so the previously reported **1308 was an over-count by 10**. The corrected figure is within one of r0038's entirely independent per-declaration enumeration, which totalled 1297 | — | — |
...
| 6 | `sorry` in the development | **0 — and still 0 after r0046**, alongside **0 `axiom` declarations** and **0 constants naming `sorryAx`**, each confirmed by two independent instruments. **Read this row with 2j: `sorry` counts holes in proofs, and cannot see a claim nobody attempted** — 7 such claims remain, recorded as `Prop`-valued `def`s by a deliberate convention that is better than a `sorry` (it asserts nothing and propagates no `sorryAx`) but was never recorded where a count would find it. As first established: **the development is `sorry`-free as of r0042.** `thm18` is proved: `PropertyM.thm18_of_cor136` applied to `JungCor136.fixedPointOfCompactDeflationIsCompact`, and `#print axioms ScottDomains.thm18` gives `[propext, Classical.choice, Quot.sound]` with no `sorryAx`. **Jung's Theorem 1.37 was not needed** — `PropertyM.forall_hasCompleteMub` derives property m from `IsAlgebraic (ScottHom D D)` and countability of `K(D → D)`, hypotheses Theorem 18 already carried, following Spreen 2005 Lemma 5.8. Five rounds treated Theorem 1.37 as the blocker; it was a strictly stronger statement than the proof needed | Formerly **1**: `thm18` at `Skeleton/Section6.lean:202`. r0034 took this from 8 to 2 and r0036 from 2 to 1 by proving Theorem 14. r0042 established that closing it is a **two-line edit**: the planned one-line `exact` was an *import cycle* — `Skeleton.Section6` sat inside `JungFinite`'s own import cone because `FinitaryProjectionPoset` imported the skeleton solely for `IsClosure` — and moving eight closure declarations to `ScottDomains/Closure.lean` removed it, which also restores the `Skeleton/` statements-only convention. Every other statement in the development is kernel-checked, and nothing in the development depends on `thm18` — the only mention outside its own file and `Section62.lean`'s obstruction write-up is a docstring reference, so no completed result routes through `sorryAx` | — |

**Round r0042 — the development is `sorry`-free.** Five agents; `thm18` closed,
taking the count 1 → **0** and the numbered results to **25 of 29**. The route
that closed it is not the one five rounds had assumed: **Jung's Theorem 1.37 was
never needed.** Spreen 2005's Lemma 5.8 proves property m *at a pair* directly,
and its `σ̃` turned out to be `JungSFP.jungFun` with a level-indexed top region —
machinery the development already had. The load-bearing measurement was that two
existing theorems asked for property m at every finite set while using it only at
`∅` and at pairs; restating them against what they use lifted the pair case to
the whole. Also **Jung's Corollary 1.36
proved**; **Iwamura's lemma and Markowsky's theorem formalized**, neither ever in
Mathlib; the Theorem 18 assembly made a library theorem instead of a script; and
`thm18`'s statement audited against the paper and found correct in all four
constituents — the first time that had been done, after five rounds of trying to
prove it. Five corrections to Jung's own write-up were recorded, the sharpest
being that **his retraction `r` is a closure, not a projection** (his formula
gives it; he never says so, and without `id ≤ i ∘ r` the proof establishes
nothing about `D`), and that **`r` is not total** — its infimum is over an empty
set for points not below the chain's largest element, so his two-case formula
needs a third. Net: `sorry` still **1**, and what remains of Theorem 18 is one
construction rather than a research programme.

**Round r0041 — stating what was never written down.** Five agents against the
62 properties r0040 found unstated, split by *missing construction* rather than
by paper section, because the gaps clustered on four absent objects. All four
landed. Two numbered results were repaired rather than extended: **Theorem 7's
second and third sentences** (closing the last numbered gap), and **Theorem 3**,
which had been proving *uniqueness of an operator never shown to exist* —
`UniformFixedPoint.lean` declined to assert existence, and `Kleene.theorem3_existsUnique`
now supplies it. Two more printed defects found: §5.2's clause 1 of the convex
pre-order is **false as printed**, and §6's "no least upper bound for `{u, u′}`"
is a typo for `{u, v}`.
