...
| 2e | — **unspecified**: paper properties with **no Lean statement at all** | **`sorry` cannot see these, which is why the row exists.** A `sorry` is a hole in a *stated* theorem; a result nobody wrote down produces no warning, no count, and no build signal. Confirmed so far: (1) **Theorem 7's second sentence** — "if `D` and `E` have effective presentations then `D → E` has one" — formalized nowhere, which is why `ComputableFunction.lean` is imported by no module and `EffectivePresentation.lean` only by it, eight theorems with no consumer (r0038, agent1). Asserted in **docstring prose only**, never stated in Lean: (2) Theorem 26 is false for a signature admitting arity 0; (3) `Colimit.Thm29Second` is false without `[Domain E]` — the countability lemma is checked, the step from it is not; (4) the `⊗`/`⊕` counterexample refuting Lemma 28's closure reading, 30 lines at `CombinatorRep.lean:504–533` whose own text says "a hand computation, not Lean-checked". **This is a lower bound, not a measurement.** No round has systematically checked which of the 100 paper properties have a Lean statement; r0038 audited the theorems that exist, which is the opposite direction. Doing it needs a pass over the paper, not over the development | — | — |
