...
| 2e | — **unspecified**: paper properties with **no Lean statement at all** | **1** — Theorem 7's second sentence, "if `D` and `E` have effective presentations then `D → E` has one", formalized nowhere. That is why `ComputableFunction.lean` is imported by no module and `EffectivePresentation.lean` only by it: eight theorems with no consumer (r0038, agent1) | **≥ 1, a lower bound.** No round has checked which of the 100 paper properties have a Lean statement — r0038 audited the theorems that *exist*, the opposite direction, so it can find a theorem serving nothing but never a property served by nothing. Closing this needs a pass over the paper | **100** |
