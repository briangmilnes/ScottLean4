...
| 5 | Modules / lines / theorems | **99 / 36275 / 1735** — measured on `main` with r0041 and r0042 merged, `Audit/` included. r0041 added 16 modules and r0042 six, among them `Iwamura.lean`, which carries **Iwamura's lemma and Markowsky's theorem — neither has ever been in Mathlib**. Previously 72 / 27892 / 1298 with r0037 merged, re-measured after r0038 by `scripts/lean-decls.py`, which `counts.sh` now calls: the grep rule it replaced counted declarations inside `/- … -/` block comments and docstring prose lines beginning "theorem"/"lemma", and missed `protected theorem`, so the previously reported **1308 was an over-count by 10**. The corrected figure is within one of r0038's entirely independent per-declaration enumeration, which totalled 1297 | — | — |
