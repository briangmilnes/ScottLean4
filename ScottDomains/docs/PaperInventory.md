...
| 2j | — **undischarged**: claims stated as `Prop`-valued `def`s that no theorem proves | **19** (r0044 agent6), of which `StepFunctionsDecidable` is one. **This is a documented convention, not an accident** — `LemThirty.Thm29Normal`'s docstring says "Recorded as a `Prop` rather than a `sorry` … the statement is fixed and citable, and nothing asserts it." So **row 6's `sorry` 0 has a systematic 19-claim blind spot by design**, and the design was never recorded where a count would find it. Six carry a nonzero "proved by" count and every such theorem is a *reduction*, not a proof. Of 88 `Prop`-valued defs, 55 lack an unconditional proof, splitting 19 claims / 36 concepts; 55 would have been the inflated figure. Census alongside: **0 `axiom` declarations**, 0 `sorryAx`, **0 of 22 structures never instantiated** | 19 | 88 |
