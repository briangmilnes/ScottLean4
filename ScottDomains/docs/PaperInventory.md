...
| 2d | — **status of the four remaining** | **Thm 18** is the development's only remaining `sorry`. Its five-step route (Jung 1989) has steps 2 and 3 proved in r0036 as `JungSFP.lemma213`, `thm214` and `lemma217`, and step 5 since r0028; steps 1 and 4 remain. **Lem 28** lists **nine** operators — `→, ⇸, ×, ⊗, +, ⊕, ()⊥, ()♯, ()♭` — and its "representable" means **p-representable over `Fp(U)`**: 2 of 9 (`×`, `()⊥`) are proved at that notion in r0036, and the three r0034 proofs at the closure reading are kernel-checked *not* to transfer. **Thm 29** splits: the first sentence is proved, and r0036 built `V` with `Domain V`, `IsBifinite V` and `V ≃o V⁺`; the universality extension step remains. **Lem 30** is over `V` and lists **ten** operators — Lemma 28's nine plus `()♮` — and remains 0 of 10 | — | — |
