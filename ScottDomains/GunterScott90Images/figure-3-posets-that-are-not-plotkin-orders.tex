...
%
% Deviations from the r0039 style contract, both settled by the orchestrator
% after this figure was reviewed as the pilot:
%   1. The contract's line "\usetikzpicture{}" is a typo - no such macro
%      exists - and is dropped here. No extra TikZ library is needed.
%   2. "solid" is drawn at minimum size 5pt against "open" at 4pt. In the scan
%      the filled dots measure about 40 px against the open circles' 28 px at
%      600 dpi, and the filled/open distinction is the whole content of the
%      caption, so it must read at a glance.
%   3. The option block also carries "obj", "arrow" and "darrow", agent2's
%      three keys for commutative diagrams, so every file in this round's set
%      has an identical preamble. This figure is a Hasse diagram and uses
%      none of the three.
