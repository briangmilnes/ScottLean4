...
- [ScottDomains/ScottDomains/Effective/A3StrictRecursive.lean](ScottDomains/ScottDomains/Effective/A3StrictRecursive.lean) — the **enumeration of `K(D ⊸ E)`**: the strict step functions are a basis of the strict function space, and `strictHom d e` is an effective presentation built from those of `D` and `E`, so `Theorem7StrictRecursive` is reduced to the same recursion theory the arrow claim needs (r0046)
