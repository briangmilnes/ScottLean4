...
- [ScottDomains/ScottDomains/Effective/A1FlatRecursive.lean](ScottDomains/ScottDomains/Effective/A1FlatRecursive.lean) — the first `RecursivePresentation`, at `N⊥`: both §3.2 conditions decided by `Primrec` witnesses rather than `Classical.dec`, plus the measurement that `Primcodable (Finset ℕ)` does exist and what the §3.2 schema claims do and do not say (r0045)
- [ScottDomains/ScottDomains/Effective/A3StrictRecursive.lean](ScottDomains/ScottDomains/Effective/A3StrictRecursive.lean) — the **enumeration of `K(D ⊸ E)`**: the strict step functions are a basis of the strict function space, and `strictHom d e` is an effective presentation built from those of `D` and `E`, so `Theorem7StrictRecursive` is reduced to the same one lemma the arrow claim needs (r0046)
- [ScottDomains/ScottDomains/Effective/A4Recursion.lean](ScottDomains/ScottDomains/Effective/A4Recursion.lean) — **§3.2's two conditions decide boundedness**: the `Denumerable (Finset _)` coding shown primitive recursive, bounded quantification lifted from `Primrec` to `Computable`, and a total unbounded search over finite normal subposets whose termination is `R47.Agent2.isNormalIn_joinClosure`; the join's index is computable too, and the paper's own `(N, s)` index is shown to need no search at all (r0049)
- [ScottDomains/ScottDomains/A1R46.lean](ScottDomains/ScottDomains/A1R46.lean) — r0046's bookkeeping: `Lemma30.Lemma30`'s universal closure refuted, and the pre-r0046 statement of `Effective.StepFunctionsDecidable` kept verbatim with the kernel's check that it implies the restated one (r0046)
- [ScottDomains/ScottDomains/Effective/A3StepDecidable.lean](ScottDomains/ScottDomains/Effective/A3StepDecidable.lean) — r0049's restatement of `Effective.StepFunctionsDecidable` over the step-function enumeration rather than over one guard, with the pre-r0049 statement kept verbatim and the direction of the change (old → new, a weakening) kernel-checked; the residue `ScottHomCRecursive` and the four claims it closes; and the round's finding — **Gunter & Scott's own index `(N, s)` needs no guard**, `consistent_stepPairs` (axiom-free) (r0049)
- [ScottDomains/ScottDomains/Effective/StrictHomCRecursive.lean](ScottDomains/ScottDomains/Effective/StrictHomCRecursive.lean) — **the `⊸` residue collapsed onto the `→` residue**: `strictHomCRecursive_of_scottHomC`, conditional and axiom-free, so `Effective.Theorem7StrictRecursive` no longer needs a root of its own. Supplies the index map `K(D ⊸ E) ↪ K(D → E)` (`strictCode`, computable from condition 1 of `d` and `e` alone), the encoding half of the `Denumerable (Finset _)` coding (`primrec_lower'`, `filterCode`, `imageCode`), `List.flatMap`/`filter`/`map` at the `Computable` level, and the transport `isNormalIn_val_image_iff` (r0053)
- [ScottDomains/ScottDomains/Lift.lean](ScottDomains/ScottDomains/Lift.lean) — the lift `D⊥` as a cpo, on Mathlib's `WithBot`
- [ScottDomains/ScottDomains/Flat.lean](ScottDomains/ScottDomains/Flat.lean) — §2.1's **flat cpo** `X⊥` with the discrete order (not `WithBot`, whose order is `X`'s): `CompletePartialOrder`, `K(X⊥) = X⊥`, `Domain` at countable `X`, `BoundedComplete`; `N⊥` and `T` as `Flat ℕ` and `Flat Bool`; monotone maps out of it are continuous
- [ScottDomains/ScottDomains/FlatPowerdomain.lean](ScottDomains/ScottDomains/FlatPowerdomain.lean) — §5.2–§5.3's worked computations: `(N⊥)♭ ≅ P N`, `(N⊥)♯ ≅ {N} ∪ P*f(N)` under superset, `(N⊥)♮` described, the corrected `⊢♮` characterization with the printed clause 1 refuted, and the three non-determinism identities separating the powerdomains
- [ScottDomains/ScottDomains/FlatSection6.lean](ScottDomains/ScottDomains/FlatSection6.lean) — §6's opening: **`(T × T)♮` is not bounded complete**, with `u′` and `v′` proved minimal upper bounds of `{u, v}` and incomparable — the conjunct Lemma 13 states for `♯` and `♭` and denies for `♮`
- [ScottDomains/ScottDomains/FlatOmega.lean](ScottDomains/ScottDomains/FlatOmega.lean) — §2.1's other two examples: `ω` is not a cpo, and a monotone `f : ω⊤ → O` that is not continuous
...
- [ScottDomains/ScottDomains/Morphism.lean](ScottDomains/ScottDomains/Morphism.lean) — §4's algebra of **maps**: `f × g` with both functor laws, joint-versus-separate Scott continuity as an `iff` (the equivalence §4.1 leaves as an exercise), `smash` surjective and `smash`/`unsmash` a projection–embedding pair, the **bistrict** predicate and its unique factorisation, `f ⊗ g` as an instance of it, `up ∘ down ⊒ id` with a witness that `⊑` fails, `f ⊕ g`, `f⊥`, `f + g`, the multiary product and coalesced sum with their universal properties, and a two-map witness that the separated sum's *continuous* completion is not unique
...
- [ScottDomains/ScottDomains/PropertyM.lean](ScottDomains/ScottDomains/PropertyM.lean) — **property m proved**, removing Jung's Theorem 1.37 and **Iwamura's lemma** from the route to **Theorem 18**: the Zorn step of property m needs only decreasing `ω`-sequences on a countable basis, and **Spreen 2005's Lemma 5.8** discharges that from the function space; `hasCompleteMub_pair`, `propertyM_pairs`, `forall_hasCompleteMub`, and `theorem_18_of_jung_corollary_1_36` — Theorem 18 with Corollary 1.36 as its only remaining hypothesis
- [ScottDomains/ScottDomains/JungFinite.lean](ScottDomains/ScottDomains/JungFinite.lean) — step 4 and the assembly of **Theorem 18**: Jung 1989's **Lemma 1.29** (property M at pairs gives it at every finite set), **König's lemma** graded by `ℕ` in place of his Rado Selection Theorem, **Lemma 2.2** (`U^∞(A)` finite), and `theorem_18_of_propertyM` — Theorem 18 with his Theorem 1.37 and Corollary 1.36 as the two named open hypotheses
- [ScottDomains/ScottDomains/Theorem18.lean](ScottDomains/ScottDomains/Theorem18.lean) — the **Theorem 18 assembly** as a library theorem: `theorem_18_of_jung_theorem_1_37_and_jung_corollary_1_36` and its sharper chain form reduce Theorem 18 to Jung's **Theorem 1.37** and **Corollary 1.36** alone, and `theorem_18_viaProjections_of_jung_theorem_1_37` states it with the paper's own conclusion via `Recovered.theorem_14`
- [ScottDomains/ScottDomains/Closure.lean](ScottDomains/ScottDomains/Closure.lean) — the shared `IsClosure` API: the cpo structure on `im(r)`, `IsClosure.apply_sSup_of_directed`, `isClosure_sSup`. Split out of `Skeleton/Section6.lean` in r0042 to take that file out of `JungFinite`'s import cone
...
- [ScottDomains/ScottDomains/Theorem29NormalInfinite.lean](ScottDomains/ScottDomains/Theorem29NormalInfinite.lean) — `Theorem29Normal` past a finite basis: Gunter's two named sufficient conditions are **equivalent** and both false at `A∞` (`hasFiniteExtensions_iff_hasNormalRealizations`, `not_hasFiniteExtensions_Ainf`); `A∞` has **infinitely many maximal points** (`infinite_maximal_Ainf`), so no "start at a later stage" repair exists; and the same fact discharges the first **infinite** case of the conclusion, `theorem_29_normal_flatBasis` (r0053)
- [ScottDomains/ScottDomains/A3Lemma30Schemes.lean](ScottDomains/ScottDomains/A3Lemma30Schemes.lean) — Lemma 30's tenth representation scheme, `(·)♮` p-representable over any carrier admitting the retraction pair (`rep_plotkin`), and the measurement that eight of Lemma 30's ten conjuncts follow from `Theorem29Normal` alone, leaving `→` and `⇸` blocked by bounded completeness rather than by a missing scheme (r0046)
...
- [ScottDomains/ScottDomains/Atomless.lean](ScottDomains/ScottDomains/Atomless.lean) — §7.3's Boolean-algebra paragraph, proved: `psi` embeds any countable poset with a least element and bounded joins onto a normal subposet of `U₀`, by a dyadic branch construction that uses neither Vaught's theorem nor a Boolean algebra; **Theorem 27 unconditionally** as `Atomless.theorem_27`
